...
# restore.sh is generated by scripts/macos/capture.sh on the old Mac and is
# gitignored, so it only exists here if it was copied over by hand.
if [ -x "$DOTFILES/scripts/macos/restore.sh" ]; then
  puts "macOS settings snapshot found. Review, then apply it:"
